...
status: active
date: YYYY-MM-DD
origin: docs/features/<feature>/spec.md  # preferred; use PRD/ideation/request path only when no living spec exists yet
...
<!-- With an origin spec or artifact, R-IDs trace to origin's when present; without one, R-IDs are derived
...
<!-- With an origin spec or artifact, scope this section to plan-time architectural choices —
...
<!-- With an origin spec or artifact, scope this section to plan-time questions; product-level
...
<!-- Each unit carries a stable plan-local U-ID (U1, U2, …) assigned sequentially.
     ...
     a gap. This anchor is what aw-work references in blockers and verification, so
