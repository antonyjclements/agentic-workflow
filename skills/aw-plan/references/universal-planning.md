...
This file is loaded when aw-plan detects a non-software task (Phase 0.1b). It replaces the software-specific phases (0.2 through 5.1) with a domain-agnostic planning workflow.
...
- **Pipeline mode?** If invoked from an automated software pipeline or any `disable-model-invocation` context: output "This is a non-software task. The automated software pipeline requires aw-work, which only supports software tasks. Use `/aw-plan` directly for non-software planning." and stop.

Once past these checks, commit to producing a plan. Do not exit because the task looks like a "lookup" or "research question" — the user invoked `aw-plan` because they want a structured output.
...
When research is recommended, do it — don't just offer. Stale recommendations (closed restaurants, rethemed attractions, outdated prices) are worse than no recommendations. The user invoked `/aw-plan` because they want a good plan, not a disclaimer about training data.
...
After structuring the plan, save it to `docs/features/<feature>/plan.md`. If the relevant feature directory does not exist, create or identify it before writing the plan.

Start the document with a `# Title` heading, followed by `Created: YYYY-MM-DD` on the next line. No YAML frontmatter.

Do not offer `/aw-work` (software-only), issue creation, or Proof review for non-software plans in the bare-bones workflow.
