...
This content is loaded when a synthesis-summary phase fires in aw-plan. There are two variants — they share structure but differ in timing and content focus:
...
- **Spec-sourced variant** (Phase 5.1.5): fires after Phase 1 research, before Phase 5.2 plan-write. Focuses on plan-time decisions (which files/modules to touch, which patterns extended vs. introduced new, test scope, refactor scope). Spec-validated WHAT is assumed and not re-stated.
...
- **Stated** — what the user said directly (in the original prompt, prior conversation, dialogue answers, or the upstream spec/origin artifact when present). Items here have explicit user-language anchors.
...
Stage 2 is what the user actually sees. The shape differs between variants because they serve different purposes — spec-sourced plans inherit a validated WHAT and surface plan-specific HOW; solo plans have no upstream and the synthesis is the WHAT.

### Spec-sourced shape (Phase 5.1.5)

Two content sections plus call-outs:

1. **Spec-scope restatement** (1-2 sentences, prose). Restates the spec's scope as orientation. The synthesis may be read days later or in parallel with other plans, so the restatement is the topic anchor that says "this is the artifact we're planning against." Stay in the spec's own vocabulary. Do NOT enumerate Implementation Units, restate constraints back at the user, or list acceptance examples.

2. **Plan-specific scoping decisions** (prose, or bullets when multi-faceted). Scope-level commitments the agent made that the spec did not: does this plan cover the full spec scope or narrow to a subset; are adjacent refactors pulled in or held out; what test scope at scenario level (which sites, which acceptance examples). Each item must pass the **affirmability test** — the user can affirm or redirect it without reading code. This section is scope claims at affirm-or-redirect level, NOT a description of where the implementation reaches, NOT PR count or commit sequencing, NOT Implementation Unit lists, NOT exact file paths or test commands — those are all plan-write outputs the synthesis cannot honestly claim. If the plan covers the full spec scope with no narrowing, expansions, or adjacent work, this section stays short ("This plan covers the full spec scope; test scope is X").
...
2. **Call outs** (zero or more, capped by plan depth). Same as spec-sourced.
...
| Plan depth | Restatement (spec-sourced) | Plan-specific scoping (spec-sourced) / Scope claim (solo) |
|---|---|---|
...
Fires only when:
- Phase 0.2 found no upstream spec or origin artifact
- AND Phase 0.4 stayed in aw-plan (did not route to aw-debug, aw-work, or universal-planning)
- AND Phase 0.5 cleared (no unresolved blockers)
- AND not on Phase 0.1 fast paths (resume normal, deepen-intent)

Each guard is an explicit conditional in SKILL.md, not implicit. R2 solo does NOT fire on resume/deepen, route-out, or spec-sourced paths.
...
Confirm and I'll proceed to research, drawing on this scope. (You can also redirect to /aw-brainstorm if this is bigger than you initially thought — I'll stop here and load it for you.)
```
...
## Spec-sourced variant (Phase 5.1.5)

Fires only when:
- Phase 0.2 found an upstream living spec or legacy brainstorm artifact
- AND not on Phase 0.1 fast paths

**Content focus**: plan-time decisions only. The spec or origin artifact already validated WHAT to build; the internal draft and stage 2 surface HOW the plan will execute that work.
...
**Reads from durable artifact body, not a synthesis section**: Phase 5.1.5 derives plan-time decisions from the spec or origin artifact body plus Phase 1 research. Older brainstorm docs that may have a legacy `## Synthesis` section work fine; that content is treated as supplementary, not authoritative, with durable body sections taking precedence.

**Why pre-write, not pre-research**: the spec or origin artifact already validated WHAT, so research is well-targeted. Plan-time decisions emerge during research and structuring (Phases 1-4), so pre-write catches them at the latest cheap moment — before Phase 5.2 commits the plan to disk.

### Stage 2 template (spec-sourced)

**Summary discipline (required):** describe **how the implementation approaches the work** at a high level — files/modules touched, patterns extended vs. introduced, scope boundaries the plan honors. Forward-looking (what *will* be in the plan), not retrospective. Spec-validated WHAT is assumed; the summary covers HOW. Form (prose, bullets, mix) and length follow the tier budget in "Stage 2: chat-time scoping synthesis" above; detail test applies per bullet.

**Anti-fluff guidance:** lead with the actual implementation shape in plain words. No qualifiers, no re-stating the spec's WHAT. If the summary just restates the problem frame, rewrite it to focus on plan-time decisions.

**Confirmation template (fires for Standard/Deep regardless of call-out count, or for any tier with one or more call-outs surviving):**

```
The spec scopes [1-2 sentence restatement of the spec's scope as orientation; in the spec's own vocabulary; NOT an enumeration of Implementation Units, constraints, or acceptance examples].

This plan [plan-specific scoping: what's covered vs. deferred vs. expanded relative to the spec; test scope; any adjacent refactors pulled in or held out. Prose or bullets per substance].
...
Confirm and I'll write the plan next, drawing on the spec, research, and this synthesis.
```
...
When the skill is invoked from an automated workflow or any `disable-model-invocation` context, the skill runs in non-interactive mode (no synchronous user). The artifact is read by downstream skills (aw-review-doc, aw-work) and human reviewers (PR review).
...
This restores the audit visibility the original design intended (un-validated bets must not propagate as authoritative content), but surfaces them under their own label rather than hiding them. Downstream review (aw-review-doc, aw-work, human PR review) can scrutinize Assumptions specifically.
...
- **Solo variant**: common redirects include "this is bigger than I thought — let me brainstorm first" (suggest `/aw-brainstorm`), "this is just a fix, no plan needed" (suggest `/aw-work`), or "I need to investigate first" (suggest `/aw-debug`).
- **Brainstorm-sourced variant**: less common, but possible — "actually this scope is wrong, take it back to brainstorm" (suggest `/aw-brainstorm` to revise the upstream doc).

In either case: stop aw-plan, suggest the alternative skill, offer to load it in-session. Don't push back or argue — the user's redirect signal is the deliberate choice.
...
- Re-statement of the entire spec or origin artifact — the synthesis is plan-perspective, not a copy
