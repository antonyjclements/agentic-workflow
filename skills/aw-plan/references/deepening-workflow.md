...
- `aw-spec-flow-analyzer` for missing user flows, edge cases, and handoff gaps
- `aw-repo-research-analyst` (Scope: `architecture, patterns`) for repo-grounded patterns, conventions, and implementation reality checks

**Context & Research / Sources & References gaps**
- `aw-learnings-researcher` for institutional knowledge and past solved problems
- `aw-framework-docs-researcher` for official framework or library behavior
- `aw-best-practices-researcher` for current external patterns and industry guidance
- Add `aw-git-history-analyzer` only when historical rationale or prior art is materially missing

**Key Technical Decisions**
- `aw-architecture-strategist` for design integrity, boundaries, and architectural tradeoffs
- Add `aw-framework-docs-researcher` or `aw-best-practices-researcher` when the decision needs external grounding beyond repo evidence

**High-Level Technical Design**
- `aw-architecture-strategist` for validating that the technical design accurately represents the intended approach and identifying gaps
- `aw-repo-research-analyst` (Scope: `architecture, patterns`) for grounding the technical design in existing repo patterns and conventions
- Add `aw-best-practices-researcher` when the technical design involves a DSL, API surface, or pattern that benefits from external validation

**Implementation Units / Verification**
- `aw-repo-research-analyst` (Scope: `patterns`) for concrete file targets, patterns to follow, and repo-specific sequencing clues
- `aw-pattern-recognition-specialist` for consistency, duplication risks, and alignment with existing patterns
- Add `aw-spec-flow-analyzer` when sequencing depends on user flow or handoff completeness

**System-Wide Impact**
- `aw-architecture-strategist` for cross-boundary effects, interface surfaces, and architectural knock-on impact
- Add the specific specialist that matches the risk:
  - `aw-performance-oracle` for scalability, latency, throughput, and resource-risk analysis
  - `aw-security-sentinel` for auth, validation, exploit surfaces, and security boundary review
  - `aw-data-integrity-guardian` for migrations, persistent state safety, consistency, and data lifecycle risks

**Risks & Dependencies / Operational Notes**
- Use the specialist that matches the actual risk:
  - `aw-security-sentinel` for security, auth, privacy, and exploit risk
  - `aw-data-integrity-guardian` for migrations, backfills, persistent data safety, constraints, transaction boundaries, and production data transformation risk (plan context — not the PR-review `aw-data-migration-reviewer` persona)
  - `aw-deployment-verification-agent` for rollout checklists, rollback planning, and launch verification
  - `aw-performance-oracle` for capacity, latency, and scaling concerns
...
```bash
SCRATCH_DIR="$(mktemp -d -t aw-plan-deepen-XXXXXX)"
echo "$SCRATCH_DIR"
```
...
1. **Summarize the agent and its target section** — e.g., "The aw-architecture-strategist reviewed Key Technical Decisions and found:"
...
- Reorder or split implementation units when sequencing is weak — but **never renumber existing U-IDs**. Reordering preserves U-IDs in their new order (e.g., U1, U3, U5 reordered is correct; renumbering to U1, U2, U3 is not). Splitting keeps the original U-ID on the original concept and assigns the next unused number to the new unit. Renumbering breaks aw-work blocker and verification references that were written against the original IDs
...
- Recommend `aw-brainstorm` if the gap is truly product-defining
