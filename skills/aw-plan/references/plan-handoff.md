...
The confidence check and aw-review are complementary:
...
When aw-review returns "Review complete", proceed to Final Checks.

**Pipeline mode:** Pipeline runs or any `disable-model-invocation` context also run `aw-review` with `mode:headless` and the plan path — the headless mode is identical to the interactive default at this phase. No further routing is offered in pipeline mode; the caller decides what to do with the returned findings. Address any P0/P1 findings before returning control to the caller.
...
2. **Run deeper doc review** - Walk through the remaining findings interactively (full aw-review walkthrough)
