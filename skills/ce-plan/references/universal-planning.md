...
- **Pipeline mode?** If invoked from an automated software pipeline or any `disable-model-invocation` context: output "This is a non-software task. The automated software pipeline requires ce-work, which only supports software tasks. Use `/ce-plan` directly for non-software planning." and stop.
...
Do not offer `/ce-work` (software-only), issue creation, or Proof review for non-software plans in the bare-bones workflow.
