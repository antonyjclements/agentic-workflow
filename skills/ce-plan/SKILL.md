...
Current year: 2026. `ce-brainstorm` defines what; `ce-plan` defines how; `ce-work` executes. Direct invocation always produces or updates a plan. Do not implement code here.
...
3. Classify domain:
   ...
   - ambiguous -> ask
4. Search `docs/brainstorms/*-requirements.md` for relevant recent origin docs. If multiple match, ask which. If one matches, treat it as primary source and carry forward actors, flows, acceptance examples, requirements, scope boundaries, decisions, dependencies, and open questions.
5. If no origin doc or input is thin, bootstrap briefly: problem, intended behavior, non-goals, success criteria, blockers. Suggest `ce-brainstorm` for product ambiguity, `ce-debug` for reachable bugs, and `ce-work` for obvious ready-to-execute fixes; the user decides.
