# AGENTS.md
<!-- AGENTIC_WORKFLOW_VERSION=0.4.0 -->
...
- Session logs live under `docs/sessions/` and are indexed by `docs/sessions/index.yml`. They are written by `aw-log-session` and consumed by `aw-synthesize-memory`.
...
- For CircleCI repos, set `post_pr.ci_monitor.provider: circleci`; `aw-monitor-pipeline` handles CircleCI natively. CircleCI-specific settings are inferred or set up via optional `docs/workflow/circleci.yml`.
- Do not put retry limits, polling cadence, or timeouts in `docs/workflow/config.yml`; `aw-monitor-pipeline` owns those details.
...
- If configured, invoke `aw-monitor-pipeline` after PR creation and let it loop until success, its own retry limit, or a genuine blocker.
...
- At the end of a meaningful session (task complete, user satisfied, or user asks to wrap up), run or offer `aw-log-session` to capture what was attempted, what worked, corrections, and dead ends.
...
- Use `aw-log-session` at the end of a meaningful session to record what was attempted, corrections made, dead ends hit, and useful sources.
...
12. Run the capture checkpoint: `aw-capture decision/learning/solution` or `aw-log-session` when appropriate. Offer `aw-log-session` at natural session ends to feed the synthesis loop.
