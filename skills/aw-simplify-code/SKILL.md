...
If `docs/standards/index.yml` exists, read it after resolving scope. Infer how it maps standards to paths/tags/domains, load only markdown standards applicable to the scoped files, and pass those standards to each reviewer. Treat relevant standards as enforceable unless they conflict with higher-priority instructions.
...
Also pass applicable `docs/standards/` excerpts. Findings based on standards should cite the standard path.
...
Review the same changes for standards violations and hacky patterns:

0. **Project standards**: changed code that violates applicable `docs/standards/` guidance
...
Do not auto-apply standards findings when the standard is ambiguous, stale, or conflicts with explicit instructions; report those separately.
...
Mention any applicable standards enforced or any standards gaps discovered.
