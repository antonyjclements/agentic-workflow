...
PRDs and raw ideas usually contain implicit ambiguity and open questions. Treat PRD and idea intake as a discovery step that resolves enough product behavior to write durable intent. If the PRD is pasted or linked, use `aw-import-prd` first so the historical source artifact is preserved, then continue from the imported PRD path. Once product behavior is explicit enough, create or update the living feature spec in the same run. If the user wants a PRD as the output, route to `aw-create-prd`.
...
- Use `aw-create-prd` when the user wants a PRD artifact. Use lightweight ideation notes only when the concept is not ready for a PRD or living spec.
...
9. Choose the output artifact:
   ...
   - PRD: route to `aw-create-prd` when the user asks to create or update a PRD
...
Create `docs/brainstorms/YYYY-MM-DD-###-slug-idea.md` when the user wants exploratory notes but not a PRD or living spec. Use `aw-create-prd` for authored PRDs.
