...
2. **Applicable `docs/standards/` guidance** -- enforce repository design/frontend standards when `docs/standards/index.yml` exists
3. **User's explicit instructions** -- override skill defaults, but call out conflicts with repo standards
4. **Skill defaults** -- apply in greenfield work or when the user asks for design guidance
...
If `docs/standards/index.yml` exists, read it and load markdown standards relevant to frontend, design, accessibility, CSS, components, or the touched paths. Treat those standards as part of context detection and verification.
...
- **Repo standards**: applicable `docs/standards/` entries for UI, accessibility, copy, testing, tokens, or component conventions
