...
- The finding's `section` is framing-level (Problem Frame, Summary, Overview, Why, Motivation, Goals — `Summary` is the new aw-plan / aw-brainstorm template heading; `Overview` retained as legacy) OR the finding explicitly questions whether a named component should exist
...
- Requirements document → `aw-plan`
- Plan document → `aw-work`
...
- Do not modify caller skills (aw-brainstorm, aw-plan, or external plugin skills that invoke aw-review-doc)
