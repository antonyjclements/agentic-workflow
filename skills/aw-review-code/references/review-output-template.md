...
- **Learnings & Past Solutions section** -- results from aw-learnings-researcher, with links to docs/solutions/ files
- **Agent-Native Gaps section** -- results from aw-agent-native-reviewer. Omit if no gaps found.
- **Deployment Notes section** -- key checklist items from aw-deployment-verification-agent. Omit if the agent did not run. Schema drift surfaces as `data-migration` findings — no separate section.
