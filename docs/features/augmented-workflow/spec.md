...
CLI surface, config keys, flags, and wiring examples for every capability in this
section live in the installed `docs/workflow/gates.md` and `docs/workflow/README.md`.
This spec states the durable contract only.
...
- Freshness gates convert LLM-driven review, compliance, capture, and memory synthesis into a deterministic, agent-free check. `aw-review`, `aw-capture`, and `aw-check-workflow-compliance` stamp their gate after completing successfully; `aw-synthesize-memory` stamps `synthesize` on every invocation, including no-op runs, because having nothing to synthesize is a valid outcome that should not leave the gate stale. A deterministic checker fails when a gate is stale by its configured mode, and passes when gates are disabled. Staleness is expressible as elapsed time, as changes to scoped paths since the stamped commit, or as both. The workflow ships the script and the freshness contract; the consumer chooses the enforcement point.
- Proof-of-work receipts close the gap where an agent could stamp a gate without running the skill. A gate cannot be stamped without recent, substantive, single-use evidence that the skill actually ran, and consuming that evidence leaves an auditable trail. A documented bypass exists for bootstrap only. Receipts remain accountability, not proof that the review was good.
- Telemetry: gate stamps can also append no-PII events to a git-tracked log, so effectiveness reporting aggregates the flywheel directly from version control rather than a service. Growth and merge contention are bounded by rotation, union merge, and retention pruning; git history is the archive.
- Skill tracking: skills record their own invocation to a git-tracked log so cross-repo analysis can answer where the workflow breaks down, whether AW is making things faster (paired with GitHub PR data), and which parts of the workflow are used most. Invocations are grouped into sessions without requiring a lifecycle hook, because target environments forbid hooks. The emitter is fire-and-forget — it never blocks or breaks the calling skill — and enterprise-safe: no network calls, no keys. The skill→workflow-step mapping is owned by the helper, not duplicated per skill, and honors per-repo step overrides.
- Org-shared knowledge: pointing the workflow at a git URL adds an org-wide learnings/standards tier alongside the repo-local ones, replacing the per-machine `~/.agents/learnings/` fallback as the second tier. Skills that read learnings or standards consult the org tier read-only; precedence is repo-local first, then org-shared.
- Org knowledge is governed content, not just a synced folder: one accountable owner, PR-reviewed changes, and self-describing entries. Agents treat org entries as advisory unless explicitly marked required, always let repo-local guidance win, surface stale or conflicting required entries to a human, and never write to the org tier — promotion of a repo-local learning to org-wide status is a human-gated PR. The governance model and templates live in `docs/workflow/org-knowledge.md`.
- The config migrator adds new default config sections to older configs while preserving existing overrides.

### Spec Traceability

- Spec traceability is installed everywhere but disabled by default.
- The deterministic checker verifies that anchors in tests and code point to real requirement IDs in living specs, that every requirement has at least one test anchor, and — when configured to require it — that a code anchor exists.
- Scoped to a change range, the checker fails when a changed anchored test is not paired with a changed owning spec, unless the commit carries an explicit override trailer.
- Traceability is not a freshness gate and is not stamped; it runs directly in pre-push or CI.
- Skills never decide whether tracing is on. They route annotation writes through the helper, which owns that policy, no-ops when disabled, and makes only explicit line-targeted edits when enabled.
...
- Workflow execution trace is installed everywhere but disabled by default.
- Structured process breadcrumbs — selected task tier, step execution, skipped steps and reasons, artifact paths — are recorded to a git-ignored local log.
- Stamping a freshness gate also records a trace event when workflow trace is enabled, so gate execution counts as process evidence without per-skill logic.
- A deterministic checker validates configured process requirements such as a recorded tier and required gate events.
...
- Behavior pinning is installed everywhere but disabled by default; the self-hosting repo enables it and carries at least one real pin.
- A pin run executes the same characterization harness against the old tree declared in a manifest and against the current checkout, and reports a machine-readable verdict.
- Pin verdicts distinguish an invalid oracle (the harness fails on old code, so it never characterized anything) from implementation drift (old passed, new failed).
- A pin check fails when one commit changes both the judged subject and its own oracle, unless the commit carries an explicit override trailer — a change cannot silently rewrite the test that judges it.
...
- `aw-work` invokes the configured e2e skill before implementation edits and runs authored specs per `e2e.run_scope`; `aw-check-workflow-compliance` reports a finding when `e2e.enabled` is true and an in-scope change carries neither e2e coverage nor a stated exception; a configured authoring skill determines who writes the tests, not whether coverage is expected.
