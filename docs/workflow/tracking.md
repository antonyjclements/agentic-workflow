...
Pass `--workflow-step` when the skill maps to a workflow step in `config.yml`; omit it for auxiliary and meta skills. Current mapping:
...
| `aw-refresh` | `refresh` |
| `aw-debug` | `debug` |
| `aw-create-worktree` | `create_worktree` |
| `aw-capture` | `capture` |
| `aw-discover-standards` | `discover_standards` |
| `aw-pin-behavior` | `pin_behavior` |
| `aw-resolve-pr-feedback` | `resolve_pr_feedback` |
| `aw-synthesize-memory` | `synthesize_memory` |
| `aw-help`, `aw-init` | *(omit)* |

Examples:

```bash
node .scripts/aw-gate.js track aw-plan --workflow-step plan
